...
module BeverageMixingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BeverageMixingResponse.new({})
    ...
    if fetch_err
      ...
      response = BeverageMixingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BeverageMixingResponse.new(fetched)
    else
