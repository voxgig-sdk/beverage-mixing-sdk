...
class BeverageEntity
  ...
  def make
    opts = @_entopts.dup
    BeverageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BeverageMixingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Beverage, Hash] the current Beverage data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BeverageMixingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Beverage fields)
  def match_get
  ...
  #
  # @param reqmatch [BeverageLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Beverage.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Beverage, Hash] the loaded Beverage; raises BeverageMixingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BeverageMixingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
