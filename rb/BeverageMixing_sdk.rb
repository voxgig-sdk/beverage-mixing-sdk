...
require_relative 'BeverageMixing_types'
...
class BeverageMixingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BeverageMixingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BeverageMixingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BeverageMixingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BeverageMixingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BeverageMixingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BeverageMixingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BeverageMixingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue BeverageMixingError => err
    ...
    if fetched.is_a?(Hash)
      status = BeverageMixingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BeverageMixingError.new(
        "graphql_error", "BeverageMixingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BeverageMixingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
