...
module BeverageMixingOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if BeverageMixingOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = BeverageMixingOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| BeverageMixingOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || BeverageMixingOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = BeverageMixingSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = BeverageMixingError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { BeverageMixingOmni.errify(err) }
