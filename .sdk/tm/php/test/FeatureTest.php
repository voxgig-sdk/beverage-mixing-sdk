...
// BeverageMixing SDK feature test
//
...
class FtCtrl extends BeverageMixingControl
{
...
class FtClient
{
    ...
    private BeverageMixingUtility $utility;

    public function __construct(BeverageMixingUtility $utility, array $options)
    {
    ...
    public function get_utility(): BeverageMixingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BeverageMixingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BeverageMixingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BeverageMixingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BeverageMixingConfig::make_feature($name);
    ...
    private static function buildUrl(BeverageMixingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BeverageMixingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BeverageMixingResponse($fetched);
        }
    ...
    private function fail(BeverageMixingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BeverageMixingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
