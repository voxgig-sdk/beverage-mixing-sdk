...
class BeverageMixingMakeError
{
    public static function call(?BeverageMixingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BeverageMixingContext([], null);
        }
        $op = $ctx->op ?? new BeverageMixingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BeverageMixingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BeverageMixingError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BeverageMixingError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof BeverageMixingError) {
