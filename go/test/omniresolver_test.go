...
import (
	...
	omni "github.com/voxgig-sdk/beverage-mixing-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BeverageMixingError); is {
				out := map[string]any{
					"name":    "BeverageMixingError",
...
func omniCtx(arg any, client *sdk.BeverageMixingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BeverageMixingSDK); is {
			client = live
