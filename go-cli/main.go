// Command BeverageMixing-cli is a CLI + REPL for the BeverageMixing SDK,
...
const prompt = "beverage-mixing"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "beverage dare"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BEVERAGE_MIXING_APIKEY (api key), BEVERAGE_MIXING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BEVERAGE_MIXING_APIKEY carries the API key and
	// BEVERAGE_MIXING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BEVERAGE_MIXING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BEVERAGE_MIXING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBeverageMixingSDK(opts)
