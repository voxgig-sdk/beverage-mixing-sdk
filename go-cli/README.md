...
boru-driven command-line client **and** interactive REPL for the BeverageMixing
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/beverage-mixing-cli)
make build

# 2. See usage (words, entities, env vars)
./beverage-mixing-cli --help

# 3. Provide credentials once, via the environment
export BEVERAGE_MIXING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./beverage-mixing-cli load 1 beverage            # {id:1} shorthand
./beverage-mixing-cli load '{id:1}' beverage       # explicit match map

# 5. Override the API base URL for a single call
BEVERAGE_MIXING_BASE=https://api.example.com ./beverage-mixing-cli load 1 beverage

# 6. No arguments -> interactive REPL
./beverage-mixing-cli
beverage-mixing> load 1 beverage
beverage-mixing> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/beverage-mixing-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/beverage-mixing-cli load 1 beverage
   ```
...
```sh
./beverage-mixing-cli load 1 beverage          # scalar shorthand for {id:1}
./beverage-mixing-cli load '{id:1}' beverage     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `beverage-mixing>`). Each line is
evaluated as its own boru expression:

```text
$ ./beverage-mixing-cli
beverage-mixing> load 1 beverage
beverage-mixing> /help
beverage-mixing> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `beverage`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/beverage-mixing-cli`. |
...
(`./beverage-mixing-cli <expr>`) and interactively (the REPL), and expressions compose the
