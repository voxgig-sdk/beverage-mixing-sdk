...
[MCP](https://modelcontextprotocol.io) server exposing the BeverageMixing SDK as
two agent tools — `beverage-mixing_list` and `beverage-mixing_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/beverage-mixing-mcp)
make build

# 2. Provide credentials via the environment
export BEVERAGE_MIXING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user beverage-mixing \
  -- /absolute/path/to/beverage-mixing-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./beverage-mixing-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// beverage-mixing_list: first page of records
{ "entity": "beverage" }
{ "entity": "beverage", "query": { } }

// beverage-mixing_load: one record by id
{ "entity": "beverage", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/beverage-mixing-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user beverage-mixing \
     -- "$PWD"/dist/*/beverage-mixing-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `beverage-mixing_list` and `beverage-mixing_load` tools now appear
   in new sessions. Ask the agent to *"list beverage using beverage-mixing"*
   and it calls `beverage-mixing_list` with `{"entity":"beverage"}`.
...
```sh
export BEVERAGE_MIXING_APIKEY=sk_live_xxx            # API key
export BEVERAGE_MIXING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `beverage-mixing_list` tool
...
### Call the `beverage-mixing_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `beverage-mixing_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `beverage-mixing_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BEVERAGE_MIXING_APIKEY` | API key sent with every request. |
| `BEVERAGE_MIXING_BASE` | Optional override of the API base URL. |
...
```sh
./beverage-mixing-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"beverage-mixing_load","arguments":{"entity":"beverage","query":{"id":1}}}}'
```
