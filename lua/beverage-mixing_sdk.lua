...
local BeverageMixingSDK = {}
BeverageMixingSDK.__index = BeverageMixingSDK
...
BeverageMixingSDK._make_feature = _make_feature


function BeverageMixingSDK.new(options)
  local self = setmetatable({}, BeverageMixingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BeverageMixingSDK:options_map()
...
function BeverageMixingSDK:get_utility()
  return Utility.copy(self._utility)
end


function BeverageMixingSDK:get_root_ctx()
  return self._rootctx
end


function BeverageMixingSDK:prepare(fetchargs)
...
function BeverageMixingSDK:direct(fetchargs)
...
-- by passing it.
function BeverageMixingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BeverageMixingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BeverageMixingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BeverageMixingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BeverageMixingSDK.new(sdkopts)
  sdk.mode = "test"
