...
local DareEntity = {}
DareEntity.__index = DareEntity


function DareEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DareEntity)
  self._name = "dare"
  self._client = client
...
function DareEntity:get_name()
  return self._name
end


function DareEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DareEntity.new(self._client, opts)
end
...
function DareEntity:mark_deleted()
  self._deleted = true
end


function DareEntity:deleted()
  return true == self._deleted
end


function DareEntity:data_set(args)
  if args ~= nil then
...
function DareEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DareEntity:match_set(args)
  if args ~= nil then
...
function DareEntity:stream(action, args, callopts)
...
---@param reqmatch DareLoadMatch
---@param ctrl? table
---@return Dare
---@return string? err
function DareEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DareEntity:_run_op(ctx, post_done)
  local utility = self._utility
