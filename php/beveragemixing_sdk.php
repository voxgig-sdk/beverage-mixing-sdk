...
class BeverageMixingSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = BeverageMixingHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BeverageMixingHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!BeverageMixingFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, BeverageMixingFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BeverageMixingUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BeverageMixingHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BeverageMixingHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BeverageMixingHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BeverageMixingError($op . "_allow",
                "BeverageMixingSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BeverageMixingHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BeverageMixingError("graphql_error",
                "BeverageMixingSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Beverage($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_beverage === null) {
                $this->_beverage = new BeverageEntity($this, null);
            }
            return $this->_beverage;
        }
        return new BeverageEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new BeverageMixingSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
