...
// BeverageMixing SDK Beverage entity
...
class BeverageEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BeverageEntity($this->_client, $opts);
    }

    /**
     * @param Beverage|array $args Beverage data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BeverageMixingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Beverage|array The current Beverage data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Beverage fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BeverageMixingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Beverage fields).
     */
    ...
    /**
     * Load a single Beverage.
     *
     * @param BeverageLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BeverageLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Beverage|array The loaded Beverage as an assoc-array at the
     *   SDK boundary; throws BeverageMixingError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BeverageMixingHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
