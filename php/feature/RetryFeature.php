...
// BeverageMixing SDK retry feature
...
class BeverageMixingRetryFeature extends BeverageMixingBaseFeature
{
    ...
    public function init(BeverageMixingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BeverageMixingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BeverageMixingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BeverageMixingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
