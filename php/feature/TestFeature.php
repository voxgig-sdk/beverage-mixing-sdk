...
// BeverageMixing SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BeverageMixingTestFeature extends BeverageMixingBaseFeature
{
    ...
    public function init(BeverageMixingContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BeverageMixingContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BeverageMixingParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BeverageMixingContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BeverageMixingContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BeverageMixingParam::call($ctx, $k);
