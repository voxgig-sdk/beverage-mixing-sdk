...
class TestDareDirect:

    def test_should_direct_load_dare(self):
        setup = _dare_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dare", "live" if setup["live"] else "unit")
        if _skip:
...
def _dare_direct_setup(mockres):
