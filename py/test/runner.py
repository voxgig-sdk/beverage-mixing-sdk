# BeverageMixing SDK test runner
...
class BeverageMixingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BeverageMixingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BeverageMixingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BeverageMixingTestRunner.getenv("BEVERAGE_MIXING_TEST_LIVE")
        override = BeverageMixingTestRunner.getenv("BEVERAGE_MIXING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BeverageMixingTestRunner.getenv(key)
        ...
        explain = BeverageMixingTestRunner.getenv("BEVERAGE_MIXING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BeverageMixingTestRunner._test_control is not None:
            return BeverageMixingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BeverageMixingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BeverageMixingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BeverageMixingTestRunner.load_test_control()
...
def load_env_local():
    BeverageMixingTestRunner.load_env_local()


def env_override(m):
    return BeverageMixingTestRunner.env_override(m)


def entity_data(v):
    return BeverageMixingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BeverageMixingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BeverageMixingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BeverageMixingTestRunner.load_test_control()


def live_client_options():
    return BeverageMixingTestRunner.live_client_options()


def live_delay_ms():
    return BeverageMixingTestRunner.live_delay_ms()
