...
//     (\`new BeverageMixingSDK(...)\` / \`BeverageMixingSDK.test(...)\` become
//     \`BeverageMixingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BeverageMixingSDK'
...
const TEST_SEED = {"entity":{"beverage":{"test01":{"id":"test01"}},"dare":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BeverageMixingSDK(...)\` and any
// \`BeverageMixingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BeverageMixingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BeverageMixingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
